...
- `awbuild/pages/` — one module per page generator: `marks.py` (the marks + fonts tear sheet;
  ...
  `prose.py` (about / links / doc pages), `changelog.py` (the Releases-sourced Changelog page).
...
in `awbuild/pages/changelog.py`, stdlib `urllib`; uses `certifi`'s CA bundle if installed, which
the python.org macOS build needs). Each release's notes render as Markdown; the list is capped at the 20 newest with a
